require'pry'
#split(\n)
#sort all the data by date
#iterate through the data to
# create guard object (regex)
# with a 0-60 array
# for each minute the guard is asleep add 1 to that minute in the 0-60 array
# select the guard and minute in which the most sleep took place.
def operation_sleepy_guard(inputs)
  sorted_data = inputs.split("\n").sort
...
class Guard
  ...
  attr_accessor :id, :minutes, :slept_most, :minute_slept_most

  def initialize(id)
    @id = id
    @minutes = Array.new(61, 0)
    @@all << self
  end
  ...
  def minute_slept_most
    @minutes.index(slept_most)
  end
...
def guards_array(inputs)
  guards_list = []
  inputs.each do |input|
    if input.include?('begins shift')
      guard_id = input.scan(/\#\d{1,}/)[0].scan(/\d{1,}/)[0]
      guards_list << guard_id
    end
  end
  guards_list.uniq!.sort!.each{|g_id| guards = Guard.new(g_id)}
  Guard
end

def set_sleeps(guard_list, sorted_data)
  guard_id = nil
  sorted_data.each_with_index do |input, index|
    if input.include?('begins shift')
      guard_id = input.scan(/\#\d{1,}/)[0].scan(/\d{1,}/)[0]
